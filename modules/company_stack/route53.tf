...
# hostname at the ALB themselves. The byoc root's dns_wiring output prints the
# record they need. Nothing else in the stack reads this record — the listener
# rule matches on the Host header, not on what resolves — so a customer-managed
